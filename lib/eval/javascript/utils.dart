...
class JsUtils {
  ...
  void init() {
    ...
    runtime.onMessage('decryptAESCryptoJS', (dynamic args) async {
      return await MBridge.decryptAESCryptoJS(
          args[0], 
          args[1], 
          args.length > 2 ? args[2] : "PKCS7"
      );
    });
...
async function decryptAESCryptoJS(encrypted, passphrase, padding = "PKCS7") {
    return await sendMessage(
        "decryptAESCryptoJS",
        JSON.stringify([encrypted, passphrase, padding])
    );
}
